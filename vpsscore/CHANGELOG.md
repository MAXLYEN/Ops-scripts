...
| 文件 | 原版本 → 当前版本 | 变更 |
| --- | --- | --- |
| `probe.sh` | 1.1.6 → 1.1.7 | 磁盘测试前检查 `/var/tmp` 剩余空间，不足 2GB 跳过并记 `disk_skip_reason`（原先直接写 512MB，快满的生产机会被写满）；dd 与 fio 的测试文件放进独立临时目录，退出或被中断时清理（fio 原先会在当前目录留下 64MB 文件）；JSON 错误输出移入私有临时目录；`probe_ver` 取自文件头（原写死 1.1.5） |
| `collect.sh` | 1.2.4 → 1.2.5 | 推送探针时远端改用 `mktemp`，不再用固定的 `/tmp/.probe.sh`；探针版本核对改为与本机固定的 ref 比较（原写死 main，固定了版本的机器会误报不一致） |
...
| 文件 | 原版本 → 当前版本 | 变更 |
| --- | --- | --- |
| `collect.sh` | 1.2.3 → 1.2.4 | 5 处 `ssh $SSH_OPTS $(ssh_args ...)` 收进 `rssh()`，参数不变；主机来源提示写出实际路径 |
