...
| 文件 | 版本 | 作用 |
| --- | --- | --- |
| `opsget` | 1.5.1 | 从仓库拉取、安装和执行运维脚本 |
| `opsbox` | 1.1.0 | 中文菜单：按分类或场景挑功能，常用组合自动串联 |
...
- **缺配置时不用敲命令**：执行前发现缺什么，就当场逐项问你——每项显示模板里的中文说明，直接输入或回车跳过，由菜单写进 `env.conf`（凭据类输入不回显，值原样保存，改之前留备份）。以后要补：`9 工具箱设置 → 3 配置管理 → 1 逐项填写`；同一处可以查看当前配置（凭据打码）。头部会提示已装功能还有几项没填
