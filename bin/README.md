...
| `opsbox` | 1.2.0 | 中文菜单：按分类或场景挑功能，常用组合自动串联 |
...
- **头部**：主机、系统、固定的版本、负载与磁盘、env.conf 状态、更新状态、待重启与遗留回滚定时器、初始化进度
- **启动时检查更新**：打开菜单就在后台把工具箱本身（引导器、公共库、菜单）和所有已装脚本逐个与固定版本比对，界面不用等；有不一致时头部显示「发现 N 个更新」，主菜单出现 `[u] 一键更新`，确认一次全部更新（旧版自动备份，菜单本身更新后自动重新载入，定时任务在用的脚本提醒手动跑一次）。5 分钟内查过不重查，换了固定版本旧结果作废。想挑着更新用 `9 工具箱设置 → 2 更新已装脚本`；命令行是 `opsget --outdated`
- **按场景找**（`?`）：从「磁盘快满了」「收不到告警邮件」这类要办的事直接找到功能
- **说明卡片**：选中功能先显示什么时候用、会做什么、等价命令，回车才执行；每次执行都打印实际调用的 `opsget` 命令
- **标记**：`[只读]` 不改动；`[需确认]` 先预演，看完再确认；`[高危]` 要手输主机名；`[联动]`、`[向导]` 是多步组合；`[未配置]` 表示本机缺配置
- **缺配置时不用敲命令**：执行前发现缺什么，就当场逐项问你——每项显示模板里的中文说明，直接输入或回车跳过，由菜单写进 `env.conf`（凭据类输入不回显，值原样保存，改之前留备份）。以后要补：`9 工具箱设置 → 4 配置管理 → 1 逐项填写`；同一处可以查看当前配置（凭据打码）。头部会提示已装功能还有几项没填
